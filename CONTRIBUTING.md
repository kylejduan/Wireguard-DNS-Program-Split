...
3. Run `./tests/run.sh` for Windows and `./tests/run-linux.sh` for Linux. Changes to Windows service recovery also need `./tests/run.sh --live`, which prompts for elevation and drives a throwaway hanging service through the real Service Control Manager. Routing/kernel changes also require the explicit disposable-VM suite described in [Linux development](docs/linux.md).
