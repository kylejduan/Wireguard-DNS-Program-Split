...
Measured on the reference workstation on September 17, 2026, 200 paired lookups of common names through the dispatcher matched direct queries to the router at the median (+0.04 ms), with +2.3 ms at p90 and +7 ms at p99 and no failures; PowerShell's per-call cost set a floor of about 10 ms on both paths.
...
The WireGuard tunnel and controller services use `Automatic` start. The controller waits for a physical IPv4 default route, adopts an early-started tunnel when available, and polls tunnel DNS until the first successful answer instead of sleeping for a fixed delay. It then enables and validates the dispatcher and NRPT path before starting dynamic WFP filters. Every 30 seconds the controller rechecks NRPT conflicts, executable and port ownership, ETW-backed loopback DNS, tunnel DNS, and the physical interface snapshot; repeated failure restarts the stack with fresh values. Each DNS probe makes up to three attempts inside one check, because a single lost datagram or transient refusal is not a dead tunnel and a restart leaves the included applications on the direct route until it completes. Before a health-check restart the controller copies the component logs to `logs\health-failures`, since the restart rewrites them.
